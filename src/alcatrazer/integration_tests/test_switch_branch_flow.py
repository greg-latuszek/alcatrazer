...
class TestSwitchBranchFlow(unittest.TestCase):
    ...
    def test_clear_then_start_repins_to_current_branch(self):
        ...
        self.assertEqual(rc, 0, "cmd_clear should succeed (Phase 9 contract)")
        self.assertFalse(
        ...
        self.assertEqual(
            ...
            "track the NEW current branch — the whole point of Phase 9. "
            f"Got: {self._pinned_branch()!r}",
        )

        # The new workspace's inner git is fresh — its Initial commit's
        # SHA is the inner_root in state.json (Phase 1 contract), and
        # the agent.txt from the previous workspace is gone.
        self.assertFalse(
