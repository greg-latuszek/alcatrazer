...
class TestNonDefaultBranchWarning(unittest.TestCase):
    """snapshot_workspace must loudly tell the user when they're on a
    non-default branch, because the snapshot comes from main/master
    regardless of what's checked out — and promoted commits land back
    on main/master, not on the current branch.
    """

    def _capture_snapshot(self, outer: str, workspace: str) -> str:
        """Run snapshot_workspace and return captured stdout."""
    ...
    def test_warns_when_outer_is_on_feature_branch(self):
        with tempfile.TemporaryDirectory() as tmp:
            outer = str(Path(tmp) / "outer")
            workspace = str(Path(tmp) / "workspace")
            make_repo(outer, branch="main")
            subprocess.run(
                ["git", "-C", outer, "checkout", "-b", "abc"],
                capture_output=True,
                check=True,
            )
            init_workspace(workspace)

            output = self._capture_snapshot(outer, workspace)

            # Mentions the user's current branch and the branch that is
            # actually being snapshotted, so the divergence is obvious.
            self.assertIn("abc", output)
            self.assertIn("main", output)

    def test_silent_when_outer_is_on_default_branch(self):
        with tempfile.TemporaryDirectory() as tmp:
            outer = str(Path(tmp) / "outer")
            workspace = str(Path(tmp) / "workspace")
            make_repo(outer, branch="main")
            ...
            # No branch-divergence warning when the user IS on the
            # default branch — otherwise the noise trains users to
            # ignore it.
            self.assertNotIn("currently on", output.lower())

    def test_silent_for_detached_head(self):
        """Detached HEAD is a power-user state; don't spam a warning."""
        with tempfile.TemporaryDirectory() as tmp:
            outer = str(Path(tmp) / "outer")
            workspace = str(Path(tmp) / "workspace")
            make_repo(outer, branch="main")
            sha = git(outer, "rev-parse", "HEAD")
            subprocess.run(
                ["git", "-C", outer, "checkout", "--detach", sha],
                capture_output=True,
                check=True,
            )
            init_workspace(workspace)
