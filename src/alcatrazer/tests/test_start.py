...
Step 3h — Dockerfile (three-stage) + entrypoint.sh generation.
"""
...
class WriteDockerfileTests(unittest.TestCase):
    """Step 3h: three-stage Dockerfile generation from coding-environment.toml."""
    ...
    def _write(self, data: dict) -> str:
        path = start.write_dockerfile(self.project_dir, data)
        self.assertEqual(path, self.project_dir / ".alcatrazer" / "Dockerfile")
        return path.read_text()

    def _slice(self, content: str, begin_marker: str, end_marker: str | None = None) -> str:
        """Return the substring between two stage markers."""
    ...
    def test_writes_to_alcatrazer_dockerfile(self):
        self._write({"languages": {"python": {"version": "3.12"}}})
        self.assertTrue((self.project_dir / ".alcatrazer" / "Dockerfile").is_file())

    def test_three_stages_in_order_dev_base_ai_base_dev(self):
        content = self._write({"languages": {"python": {"version": "3.12"}}})
    ...
    def test_dev_stage_mise_use_for_non_default_manager(self):
        content = self._write({"languages": {"python": {"version": "3.12", "manager": "uv"}}})
        dev = self._slice(content, "FROM ai-base AS dev")
...
class WriteEntrypointShTests(unittest.TestCase):
    """Step 3h: entrypoint.sh is byte-exact copied from the package template."""
    ...
    def test_writes_to_alcatrazer_dir(self):
        path = start.write_entrypoint_sh(self.project_dir)
        self.assertEqual(path, self.project_dir / ".alcatrazer" / "entrypoint.sh")
        self.assertTrue(path.is_file())

    def test_content_matches_package_template_bit_exact(self):
        source = Path(start.__file__).parent / "container" / "entrypoint.sh"
        path = start.write_entrypoint_sh(self.project_dir)
        self.assertEqual(
