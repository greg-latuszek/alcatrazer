...
tree into .alcatrazer/src/alcatrazer/; Dockerfile generator (three
stages: dev-base + ai-base static, dev generated from toml) and
entrypoint.sh copier. Workspace name generation and its persistence to
.alcatrazer/workspace-dir are covered by alcatrazer.identity. Steps
3i-3k fill in the remaining first-time work. Step 4 handles
subsequent-run.
"""
...
def write_dockerfile(project_dir: Path, data: dict) -> Path:
    """Generate .alcatrazer/Dockerfile from a coding-environment dict."""
    alcatrazer_dir = project_dir / ".alcatrazer"
    alcatrazer_dir.mkdir(parents=True, exist_ok=True)
    target = alcatrazer_dir / "Dockerfile"
    target.write_text(_render_dockerfile(data))
    return target


def write_entrypoint_sh(project_dir: Path) -> Path:
    """Copy the packaged entrypoint.sh byte-exact into .alcatrazer/."""
    alcatrazer_dir = project_dir / ".alcatrazer"
    alcatrazer_dir.mkdir(parents=True, exist_ok=True)
    source = Path(__file__).parent / "container" / "entrypoint.sh"
    target = alcatrazer_dir / "entrypoint.sh"
