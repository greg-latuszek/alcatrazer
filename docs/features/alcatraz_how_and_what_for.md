...
The full specifications for `coding-environment.toml` format, `.alcatrazer/config.toml`
schema, CLI commands, error handling, and implementation plan have been migrated to
[install_method.md](install_method.md) — the actionable build plan.

What remains below is the architectural reasoning that led to these decisions.

### Language runtime installation: mise

**Decision:** Language runtimes are installed via `mise` — a multi-language version manager
already present in the alcatrazer base layer (`src/alcatrazer/container/Dockerfile`).
...
**apt-get** was rejected — version availability limited to what the distro ships.
**Language-specific managers** were rejected — complexity with no benefit over mise.
**mise covers:** Python, Node.js, Ruby, Go, Rust, Java, Bun, Deno, PHP, Erlang/Elixir,
.NET, Zig, Lua, R, and non-language tools.
